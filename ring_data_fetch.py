...
def fetch_snowflake_ring_data(query, local_folder, add_prefix=""):
    """
    Fetch data from Snowflake using a query and download to local directory.
    ...
    local_folder : str
        Local directory path where data will be downloaded
    add_prefix : str, optional
        Additional prefix for downloaded files
    
    Returns:
    --------
    None (downloads files to local directory)
    """
    ...
    def snowflake_read_connector(input_role, input_warehouse, input_database=None, input_schema=None):
        ...
        with open("/home/ubuntu/temp_data/credentials/RING_ANALYTICS_AUTOMATION_rsa_key.p8.der", "rb") as key_file:
            private_key = key_file.read()
        
        connection_params = {
            "user": "RING_ANALYTICS_AUTOMATION",
    ...
    con = snowflake_read_connector(
        input_role='RING_ANALYTICS_PROGRAMATIC_ACCESS_EDIT_ONLY_ROLE',
        input_warehouse='RING_ANALYTICS_PROGRAMATIC_ACCESS_EDIT_ONLY_ROLE_WH'
    )
    
    engine = snowflake_write_engine(
        input_role='RING_ANALYTICS_PROGRAMATIC_ACCESS_EDIT_ONLY_ROLE',
        input_warehouse='RING_ANALYTICS_PROGRAMATIC_ACCESS_EDIT_ONLY_ROLE_WH',
        input_database='RING_REPORTS',
...
if __name__ == "__main__":
    ...
    inner join
    ring_source.bureau_scrub.equifax_output c
    on a.user_reference_number = c.reference_no
    """
    
    fetch_snowflake_ring_data(
        query=base_query,
        local_folder="/home/manu.chandra/daily_tracking/daily_logs_generation/temp_data/ring_data/"
    )